...
var config = {
  ...
  recaptcha_sitekey: '6LfXwhETAAAAAMevh4Ua9wdm2pBWLA9_9ZEMxPqa',  // <----- EDIT ME!
...
var betStore = new Store('bet', {
  ...
  Dispatcher.registerCallback('UPDATE_WAGER', function(newWager) {
    ...
    var n = parseFloat(self.state.wager.str); //parseInt(self.state.wager.str, 10);
    ...
    if (isNaN(n) || /^-?\d+((.\d)?\d*(e[-]?\d)?(\d)*)$/.test(n.toString())) { // - /[^\d]/.test(n.toString())) {
    ...
    return el.div(
      ...
      el.p(
        ...
        el.strong(
          ...
          'Wager: (' + betStore.state.wager.error)
      ),
      el.input(
        {
